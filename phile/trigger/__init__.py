...
class EventFilter:
    """
    Ignore events that are not trigger activations.

    An event is a trigger activation if it deletes a file
    with the :attr:`~phile.configuration.Configuration.trigger_suffix`
    and is in the ``trigger_directory``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        :param ~phile.configuration.Configuration configuration:
            Information on what constitute a trigger file.
        :param event_handler:
        ...
        """Callback repsonsible of processing triggers."""
        self._trigger_suffix = configuration.trigger_suffix
        """Suffix that triggers must have."""
    ...
    def __call__(
        ...
        Calls ``event_handler``
        if ``watchdog_event`` is a trigger activation.
        """
        ...
        if event_type != watchdog.events.EVENT_TYPE_DELETED:
            return
        trigger_path = pathlib.Path(watchdog_event.src_path)
        if trigger_path.suffix != self._trigger_suffix:
            return
        if trigger_path.parent != self._trigger_directory:
            return
        self._event_handler(watchdog_event)


class EventConverter:
    """Convert a trigger event to the name of trigger activated."""
    ...
    def __init__(
        ...
        *,
        ...
        :param ~phile.configuration.Configuration configuration:
            Information on what constitute a trigger file.
        :param trigger_handler:
            Handler to call with the trigger name.
        :type trigger_handler: :data:`~Handler`
        """
        self._trigger_handler = trigger_handler
        """Callback repsonsible of processing trigger by name"""
        self._trigger_suffix = configuration.trigger_suffix
        """Suffix that triggers must have."""
    ...
    def __call__(
        ...
        Calls ``trigger_handler`` with the name of trigger.
        The given ``watchdog_event`` must be a trigger activation event,
        as specifed by :class:`EventFilter`.
        It is undefined behaviour if it is not.
        """
        trigger_path = pathlib.Path(watchdog_event.src_path)
        trigger_file_name = trigger_path.name
        assert trigger_file_name.endswith(self._trigger_suffix)
        trigger_name = trigger_file_name[:-len(self._trigger_suffix)]
        self._trigger_handler(trigger_name)

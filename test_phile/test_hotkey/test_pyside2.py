...
from test_phile.test_PySide2.test_QtCore import UsesQCoreApplication
...
class TestFuture(unittest.TestCase):
    """Tests :class:`~phile.hotkey.pyside2.Future`."""
    ...
    def done_callback(
        self, future: phile.hotkey.pyside2.Future[int]
    ) -> None:
        del future
        self.done = True

    def test_static_check_add_done_callback(self) -> None:
        future = phile.hotkey.pyside2.Future[int]()
...
class TestTask(unittest.TestCase):
    """Tests :class:`~phile.hotkey.pyside2.Task`."""
    ...
    def test_run_sets_result(self) -> None:
        ...
        task = phile.hotkey.pyside2.Task[int](callback=zero)
        task.run()
    ...
    def test_run_propagates_exception(self) -> None:
        ...
        task = phile.hotkey.pyside2.Task[int](callback=bad)
        task.run()
...
class TestExecutor(UsesQCoreApplication, unittest.TestCase):
    """Tests :class:`~phile.hotkey.pyside2.Executor`."""

    def test_calls_submitted_callable(self) -> None:
        callback_mock = unittest.mock.Mock()
        with phile.hotkey.pyside2.Executor() as executor:
...
class UsesExecutor(UsesCapabilities, unittest.TestCase):
    ...
    def setUp(self) -> None:
        super().setUp()
        self.executor = phile.hotkey.pyside2.Executor()
